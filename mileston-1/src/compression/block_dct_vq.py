    ...
    def compress(
        ...
        *,
        ...
        if output_path:
            ...
            # If no output path, we can't save/measure bytes, but we can return the image object in a modified stats object?
            # For now, let's assume output_path is provided as per usage in demo_runner.
            # If not, we set bytes to 0 and output_path to None (though the type hint says Path)
            bytes_on_disk = 0
